              ...
              {favorites.map((movie, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-3 items-start">
                      <Link to={`/movie/${movie.id}`} className="shrink-0">
                        <img
                          ...
                          alt={movie.title}
                          className="w-26 h-39 md:w-45.5 md:h-67.5 object-cover rounded-lg md:rounded-xl"
                        />
                      </Link>
                      ...
                      <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                        ...
                        <Link
                          to={`/movie/${movie.id}`}
                          className="hidden md:inline-flex mt-2 items-center justify-center gap-2 self-start w-50 h-13 bg-primary hover:bg-primary/90 text-white font-semibold text-sm rounded-full transition-colors"
                        >
                          Watch Trailer
                          <PlayCircle size={16} />
                        </Link>
                    ...
                    {/* Heart — col 2, always in the same column */}
                    <button
                      onClick={() => removeFromFavorites(movie.id)}
                      className="hidden md:flex shrink-0 w-11 h-11 items-center justify-center rounded-full bg-zinc-900 border border-zinc-700 hover:border-zinc-500 transition-colors"
                    ...
                    <div className="flex md:hidden gap-3">
                      <Link
                        to={`/movie/${movie.id}`}
                        className="flex-1 flex items-center justify-center gap-2 h-11 bg-primary hover:bg-primary/90 text-white font-semibold text-sm rounded-full transition-colors"
                      >
                        Watch Trailer
                        <PlayCircle size={16} />
                      </Link>
